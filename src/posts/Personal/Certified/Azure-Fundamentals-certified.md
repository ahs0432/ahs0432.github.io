...
## 시험 방식
시험 방식은 크게 온라인과 오프라인으로 나뉘어 진행되는 구조를 가지고 있다.  

많은 외국계 시험에서 이용하고 있는 `Pearson Vue`를 통해 시험을 진행하게 된다.  
필자는 온라인을 통해 집에서 편안하게 시험을 응시했고 이에 대한 후기를 작성하려고 한다.

### 온라인
온라인은 경험 상으로 아래와 같은 절차로 이뤄진다고 볼 수 있다.
...
4. 시험 응시를 위한 환경 테스트
5. 신분 확인 및 시험 환경 평가
- 신분증 사진과 시험 환경 사진을 찍어 제출한다.
- 제출 이후 대기할 경우 감독관이 내용을 확인하고 입실한다.
5. 시험 응시 및 결과 확인

#### 장점
1. 내가 마련해둔 환경이 있다면 편안한 환경에서 시험을 볼 수 있다는 것이다.
2. 응시할 수 있는 시험이 15분 단위로 존재하기 떄문에 시간의 구애를 받지 않는다.
- 0/15/30/45 이렇게 15분 단위로 시험이 있고 취소하는 인원도 있기에 나름 널널하다.

#### 단점
1. 주어진 환경이 없다면 아무래도 환경을 마련하는데에 스트레스를 받을 것 같다.
- 또한 환경 검사를 하는 감독관마다 성향이 달라 검사하는 범위의 차이가 있다.
2. 감독관의 언어가 한국어를 지원하지 않기 때문에 이로 인한 곤욕을 치룰 수 있다.
- 감독관의 마이크가 계속 깨져들려서 `Chat plz`라고 이야기 해서 채팅으로도 받아봤다.
3. 여권 등 국제적으로 인증이 가능한 신분증이 없는 경우 시험 응시에 어려움이 있다.
- 운전면허증도 가능하다고 하는데 영문 이름이 없는 경우 어려움이 있을 수 있다.

### 오프라인
오프라인은 응시해본 적이 없기에 정확히 알 수 없지만 방식은 대강 아래와 같이 이뤄진다.

1. 원하는 일자에 시험 일정을 예약한다.
- 시험 장소에 따라 제공되는 일정이 다를 수 있기에 주의해서 신청한다.
2. 국가에서 인정하는 신분증을 가지고 시험을 응시한다.
- 온라인과 다르게 주민등록증으로도 인증이 가능하니 참고하도록 하자.

#### 장점
1. 별도의 장소를 마련할 필요없이 이미 구축된 장소에서 시험을 볼 수 있다.
2. 신분증을 준비하는 것이 국제 표준을 지켜야하는데 이는 그럴 필요가 없다.
3. 한국인이 감독관으로 있기에 더 편한 환경(?)에서 시험을 응시할 수 있다.
