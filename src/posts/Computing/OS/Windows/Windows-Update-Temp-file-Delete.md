...
![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/1.png =90%x90%)
- `Windows Update` 상태가 최신이 아님에도 신규 업데이트를 받아오지 못하는 현상이 확인된다고 가정합니다.

### Windows Update 관련 프로세스 종료
  
![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/2.png =45%x45%)
- 실행창(`Windows` + `R`)을 열고 실행창에 `services.msc`를 입력하여 서비스 창을 열어줍니다.

![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/3.png =90%x90%)
...
![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/4.png =90%x90%)
- `C:\Windows` 경로로 이동하여 `SoftwareDistribution` 폴더를 삭제합니다.

![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/5.png =90%x90%)
- `C:\Windows\System32` 경로에 `catroot2` 폴더를 삭제합니다.
- 해당 폴더 삭제 후 다시 `services.msc` 창으로 돌아와 중지하였던 서비스를 시작합니다.

![](/assets/image/Post/Computing/OS/Windows/Windows-Update-Temp-file-Delete/6.png =60%x60%)
